...
const (
	...
	UserSideSecuritySupportEnv = "GRPC_XDS_EXPERIMENTAL_SECURITY_SUPPORT"
...
var (
	...
	// XDSUserSideSecurity is used to control processing of security
	// configuration on the User-side.
	//
	// Note that there is no env var protection for the server-side because we
	// have a brand new API on the server-side and users explicitly need to use
	// the new API to get security integration on the server.
	XDSUserSideSecurity = !strings.EqualFold(os.Getenv(UserSideSecuritySupportEnv), "false")
